...
package com.newstoday.news_package.news_category.two.fragment;
...
import com.newstoday.news_package.news_category.two.adapter.EntriesCursorAdapter;
...
import com.newstoday.news_package.news_category.two.service.FetcherService;
...
public class EntriesListFragment extends SwipeRefreshListFragment implements ViewTreeObserver.OnScrollChangedListener {
    ...
    public void inflateView(LayoutInflater inflater, ViewGroup container) {
        ...
        boolean isFirst = Pref_Util_Service.getPrefBoolean(getActivity(), "cat_Two", true);
        if (isFirst) {
            Toast.makeText(getActivity(), getResources().getString(R.string.refresh_suggestion), Toast.LENGTH_LONG).show();
            Pref_Util_Service.putPrefBoolean(getActivity(), "cat_Two", false);
            startRefresh();
        }
    }
