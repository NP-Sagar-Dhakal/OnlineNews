...
public class Radio_Recycler_Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    Radio_Recycler_Adapter(Context context, List<NewsItem.News.OnlineRadios> radioItems) {
        ...
        Radio_Recycler_Adapter.radioItems = radioItems;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holderr, int position) {
        ...
        TextDrawable letterDrawable = TextDrawable.builder().buildRoundRect((radioItems.get(position).stationName != null ? radioItems.get(position).stationName.substring(0, 1).toUpperCase() : ""), color, 8);
        try {
