...
public class HtmlUtils {
    ...
    public static ArrayList<String> getImageURLs(String content) {
        ...
        if (!TextUtils.isEmpty(content)) {
            ...
            while (matcher.find()) {
                images.add(Objects.requireNonNull(matcher.group(1)).replace(" ", URL_SPACE));
            }
        }
    ...
    public static String replaceImageURLs(String content, final long entryId) {
        ...
        if (!TextUtils.isEmpty(content)) {
            ...
            Matcher matcher = IMG_PATTERN.matcher(content);
            while (matcher.find()) {
                String match = Objects.requireNonNull(matcher.group(1)).replace(" ", URL_SPACE);
    ...
    public static String getMainImageURL(String content) {
        if (!TextUtils.isEmpty(content)) {
            ...
            while (matcher.find()) {
                String imgUrl = Objects.requireNonNull(matcher.group(1)).replace(" ", URL_SPACE);
